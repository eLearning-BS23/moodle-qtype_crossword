    ...
    \core_privacy\local\request\user_preference_provider
{
    ...
    public static function export_user_preferences(int $userid)
    {
        ...
        if (null !== $preference) {
            ...
            writer::export_user_preference('qtype_crossword', 'defaultmark', $preference, $desc);
        }

        $preference = get_user_preferences('qtype_crossword_penalty', null, $userid);
        if (null !== $preference) {
            $desc = get_string('privacy:preference:penalty', 'qtype_crossword');
            writer::export_user_preference('qtype_crossword', 'penalty', transform::percentage($preference), $desc);
        }

        $preference = get_user_preferences('qtype_crossword_shuffleanswers', null, $userid);
        if (null !== $preference) {
            $desc = get_string('privacy:preference:shuffleanswers', 'qtype_crossword');
            writer::export_user_preference('qtype_crossword', 'shuffleanswers', transform::yesno($preference), $desc);
        }
    }
}
